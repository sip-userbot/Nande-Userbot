        ...
        async def wrapper(client, message):
            ...
            if ignore_errors:
                ...
                try:
                    ...
                    text += "\n\n__You can Forward This to @Nandesupport, If You Think This is Serious A Error!__"
                    try:
    ...
    def decorator(func):
        async def wrapper(client, message):
            try:
                ...
                text += f"\n**Trace Back : ** `{str(format_exc())}`"
                text += f"\n**Nande - Userbot-Name :** `{func.__module__}`"
                text += f"\n**Function Name :** `{func.__name__}` \n"
                text += datetime_tz.strftime(
                    "**Date :** `%Y-%m-%d` \n**Time :** `%H:%M:%S`"
                )
                text += "\n\n__You can Forward This to @Nandesupport, If You Think This is A Error!__"
                try:
                    await client.send_message(Config.LOG_GRP, text)
                except BaseException:
                    logging.error(text)
        ...
        async def wrapper(client, message):
            ...
            if ignore_errors:
                ...
                try:
                    ...
                    text += "\n\n__You can Forward This to @suportsipuserbot, If You Think This is Serious A Error!__"
                    try:
                        await client.send_message(Config.LOG_GRP, text)
                    except BaseException:
                        logging.error(text)
