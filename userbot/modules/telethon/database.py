...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        nande = await edit_or_reply(var, "`Getting information...`")
        await asyncio.sleep(1.0)
        try:
            variable = var.pattern_match.group(2).split()[0]
            if variable in heroku_var:
                return await nande.edit(
        ...
        await nande.edit(f"`{variable}`  **successfully deleted**")
...
async def nande(event):  # sourcery no-metrics
