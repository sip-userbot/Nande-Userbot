...
async def _(event):
    ...
    try:
        ...
        await nandeevent.edit(str(e))
    else:
        if photo:
            await nandeevent.edit("`now, Uploading to Telegram ...`")
            if photo.endswith((".mp4", ".MP4")):
                # https://t.me/tgbetachat/324694
                size = os.stat(photo).st_size
                if size > 2097152:
                    await nandeevent.edit("`size must be less than 2 mb`")
                    os.remove(photo)
                    return
            ...
            try:
                ...
                await edit_or_reply(
                    nandeevent, "`My profile picture was succesfully changed`"
                )
    try:
        os.remove(photo)
    except Exception as e:
...
async def count(event):
    ...
    b = 0
    result = ""
    nandeevent = await edit_or_reply(event, "`Processing..`")
