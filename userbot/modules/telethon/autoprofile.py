...
async def autopicloop():
    ...
    if AUTOPICSTART and Config.DEFAULT_PIC is None:
        if BOTLOG:
            return await nandeub.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await nandeub.upload_file(autophoto_path)
        try:
            await nandeub(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        img = Image.open(autophoto_path)
        drawn_text = ImageDraw.Draw(img)
        nande = str(base64.b64decode("TmFuZGUvaGVscGVycy9zdHlsZXMvZGlnaXRhbC50dGY"))[
            2:36
        ]
        fnt = ImageFont.truetype(nande, 200)
            ...
            if i > 0:
                await nandeub(
                    functions.photos.DeletePhotosRequest(
                        await nandeub.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await nandeub(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await nandeub(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await nandeub(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        file = await nandeub.upload_file("donottouch.jpg")
        if i > 0:
            await nandeub(
                functions.photos.DeletePhotosRequest(
                    await nandeub.get_profile_photos("me", limit=1)
...
nandeub.loop.create_task(autopfp_start())
nandeub.loop.create_task(autopicloop())
nandeub.loop.create_task(digitalpicloop())
nandeub.loop.create_task(bloom_pfploop())
nandeub.loop.create_task(autoname_loop())
nandeub.loop.create_task(autobio_loop())
