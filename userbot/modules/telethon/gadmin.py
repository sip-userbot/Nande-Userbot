...
async def nandegban(event):  # sourcery no-metrics
    "To ban user in every group where you are admin."
    nandee = await edit_or_reply(event, "`gbanning.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, nandee)
    if not user:
        return
    if user.id == nandeub.uid:
        return await edit_delete(nandee, "`why would I ban myself`")
    if gban_sql.is_gbanned(user.id):
        await nandee.edit(
            f"`the `[user](tg://user?id={user.id})` is already in gbanned list any way checking again`"
        )
    else:
        gban_sql.nandegban(user.id, reason)
    san = []
    san = await admin_groups(event)
    count = 0
    sandy = len(san)
    if sandy == 0:
        return await edit_delete(nandee, "`you are not admin of atleast one group` ")
    await nandee.edit(
    ...
    nandetaken = (end - start).seconds
    if reason:
        await nandee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {nandetaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await nandee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {nandetaken} seconds`!!"
...
async def nandegban(event):
    "To unban the person from every group where you are admin."
    nandee = await edit_or_reply(event, "`ungbanning.....`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, nandee)
    if not user:
        return
    if gban_sql.is_gbanned(user.id):
        gban_sql.nandeungban(user.id)
    else:
        return await edit_delete(
            nandee, f"the [user](tg://user?id={user.id}) `is not in your gbanned list`"
        )
    san = []
    san = await admin_groups(event)
    count = 0
    ...
    if reason:
        await nandee.edit(
            f"[{user.first_name}](tg://user?id={user.id}`) was ungbanned in {count} groups in {nandetaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await nandee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was ungbanned in {count} groups in {nandetaken} seconds`!!"
...
async def nandegkick(event):  # sourcery no-metrics
    "kicks the person in all groups where you are admin"
    nandee = await edit_or_reply(event, "`gkicking.......`")
    start = datetime.now()
    ...
    if reason:
        await nandee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {nandetaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await nandee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {nandetaken} seconds`!!"
