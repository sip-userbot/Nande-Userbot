...
from ...helpers.utils import _nandetools
...
def verify_cond(nandearray, text):
    return any(i in text for i in nandearray)


def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"nandeuserbot_{userid}_{pack}_anim"
    return f"nandeuserbot_{userid}_{pack}"
...
async def kang(args):  # sourcery no-metrics
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            nandeevent = await edit_or_reply(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        if (
            ...
            await nandeevent.edit("`Brewing a new Pack...`")
            async with args.client.conversation("Stickers") as conv:
...
async def pack_kang(event):  # sourcery no-metrics
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await edit_delete(nandeevent, "`Unsupported File!`")
            return
        if photo:
            ...
            if (
                ...
                async with event.client.conversation("Stickers") as conv:
                    pack, nandepackname = await newpacksticker(
                        nandeevent,
                        conv,
                        cmd,
                        event,
                ...
                async with event.client.conversation("Stickers") as conv:
                    pack, nandepackname = await add_to_pack(
                        nandeevent,
                        conv,
                        event,
                        packname,
            ...
            if nandepackname not in blablapacks:
                blablapacks.append(nandepackname)
...
async def pic2packcmd(event):
    ...
    try:
        ...
        emoji = "▫️"
    chat = "@Stickers"
    name = "NandeUserbot_" + "".join(
        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)
    )
    image = await _nandetools.media_to_pic(nandeevent, reply, noedits=True)
    if image[1] is None:
        return await edit_delete(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    nandeevent = await event.edit("__Making the pack.__")
...
async def get_pack_info(event):
    ...
    try:
        ...
        nandeevent = await edit_or_reply(event, "`sedang mencari, Mohon tunggu..`")
    except BaseException:
        return await edit_delete(event, "`Mohon balas ke sticker.`", 5)
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await nandeevent.edit("`Mohon balas ke sticker Nande.`")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
...
async def cb_sticker(event):
    ...
    nandeevent = await edit_or_reply(event, "`Mencari  sticker packs Nande....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "Sticker-pack__Nande"})
    if not results:
        return await edit_delete(nandeevent, "`Tidak dapat ditemukan :(.`", 5)
    reply = f"**Sticker Pack Nande {split} are :**"
    for pack in results:
        if pack.button:
