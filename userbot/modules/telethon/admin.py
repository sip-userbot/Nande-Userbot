...
async def promote(event):
    ...
    if not user:
        return
    nandeevent = await edit_or_reply(event, "`Promoting...`")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await nandeevent.edit(NO_PERM)
    await nandeevent.edit("`Promoted Successfully! Now gib Party`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#PROMOTE\
...
async def demote(event):
    ...
    if not user:
        return
    nandeevent = await edit_or_reply(event, "`Demoting...`")
    ...
    try:
        ...
        return await nandeevent.edit(NO_PERM)
    await nandeevent.edit("`Demoted Successfully! Betterluck next time`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#DEMOTE\
...
async def _ban_person(event):
    ...
    nandeevent = await edit_or_reply(event, "`Whacking the pest!`")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    nandeevent = await edit_or_reply(event, "`Unbanning...`")
    try:
        ...
        await nandeevent.edit("`Uh oh my unban logic broke!`")
    except Exception as e:
        await nandeevent.edit(f"**Error :**\n`{e}`")
...
async def endmute(event):
    ...
    if not user:
        return
    nandeevent = await edit_or_reply(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await nandeevent.edit(NO_PERM + f"\n{str(e)}")
    if reason:
        await nandeevent.edit(
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    nandeevent = await edit_or_reply(event, "`Mencari tindakan terbaru .....`")
    ...
    if not flag:
        ...
        await edit_or_reply(nandeevent, deleted_msg)
    else:
