...
from ...helpers.utils import _nandeutils, reply_id
...
async def _(event):
    ...
    nande = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    nandeevent = await edit_or_reply(event, "`wi8..! I am finding your song....`")
    video_link = await yt_search(str(query))
    if not url(video_link):
        return await nandeevent.edit(
    ...
    try:
        nande = Get(nande)
        await event.client(nande)
    except BaseException:
        pass
    stderr = (await _nandeutils.runcmd(song_cmd))[1]
    if stderr:
        return await nandeevent.edit(f"**Error :** `{stderr}`")
    nandename, stderr = (await _nandeutils.runcmd(name_cmd))[:2]
    if stderr:
        return await nandeevent.edit(f"**Error :** `{stderr}`")
    # stderr = (await runcmd(thumb_cmd))[1]
    nandename = os.path.splitext(nandename)[0]
    # if stderr:
    #    return await nandeevent.edit(f"**Error :** `{stderr}`")
    song_file = Path(f"{nandename}.mp3")
    if not os.path.exists(song_file):
        return await nandeevent.edit(
            f"Sorry!. I can't find any related video/audio for `{query}`"
        )
    await nandeevent.edit("`yeah..! i found something wi8..🥰`")
    nandethumb = Path(f"{nandename}.jpg")
    if not os.path.exists(catthumb):
        nandethumb = Path(f"{nandename}.webp")
    elif not os.path.exists(nandethumb):
        pass
    ...
    for files in (nandethumb, song_file):
...
async def _(event):
    ...
    stderr = (await _nandeutils.runcmd(video_cmd))[1]
    if stderr:
        return await nandeevent.edit(f"**Error :** `{stderr}`")
    nandename, stderr = (await _catutils.runcmd(name_cmd))[:2]
    if stderr:
        return await nandeevent.edit(f"**Error :** `{stderr}`")
    # stderr = (await runcmd(thumb_cmd))[1]
    try:
    ...
    # if stderr:
    #    return await nandeevent.edit(f"**Error :** `{stderr}`")
    nandename = os.path.splitext(catname)[0]
    vsong_file = Path(f"{nandename}.mp4")
    if not os.path.exists(vsong_file):
        vsong_file = Path(f"{nandename}.mkv")
    ...
    await nandeevent.edit("`yeah..! i found something wi8..🥰`")
    nandethumb = Path(f"{nandename}.jpg")
    if not os.path.exists(nandethumb):
        nandethumb = Path(f"{nandename}.webp")
    elif not os.path.exists(nandethumb):
        nandethumb = None
    await event.client.send_file(
    ...
    await nandeevent.delete()
    for files in (nandethumb, vsong_file):
        if files and os.path.exists(files):
            os.remove(files)
...
async def shazamcmd(event):
    ...
    nandeevent = await edit_or_reply(event, "__Downloading the audio clip...__")
    try:
        ...
        LOGS.error(e)
        return await edit_delete(
            nandeevent, f"**Error while reverse searching song:**\n__{str(e)}__"
        )
...
async def _(event):
    ...
    nandeevent = await edit_or_reply(event, SONG_SEARCH_STRING, parse_mode="html")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if baka[0].message.startswith(
                ...
                return await edit_delete(
                    nandeevent, SONG_NOT_FOUND, parse_mode="html", time=5
                )
            await nandeevent.edit(SONG_SENDING_STRING, parse_mode="html")
            ...
            return await nandeevent.edit(SONGBOT_BLOCKED_STRING, parse_mode="html")
        await event.client.send_file(
            event.chat_id,
            music,
...
async def _(event):
    ...
    chat = "@auddbot"
    nandeevent = await edit_or_reply(event, "```Identifying the song```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await nandeevent.edit("Wait just a sec...")
            result = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            await nandeevent.edit("```Please unblock (@auddbot) and try again```")
            return
    namem = f"**Song Name : **`{result.text.splitlines()[0]}`\
        \n\n**Details : **__{result.text.splitlines()[2]}__"
    await nandeevent.edit(namem)
