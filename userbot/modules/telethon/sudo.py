...
async def chat_blacklist(event):
    "To enable or disable sudo of your NandeUserbot."
...
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "__There are no sudo users for your Nandeuserbot.__"
        )
    result = "**The list of sudo users for your Nandeuserbot are :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "-all":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += f"`{cmd}` __There is no such plugin in your NandeUserbot__.\n"
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += (
                    f"`{cmd}` __There is no such command in your NandeUserbot__.\n"
                )
    ...
    result = (
        f"__Successfully enabled __ `{len(loadcmds)}` __ for NandeUserbot sudo.__\n"
    )
    output = (
    ...
    if errors != "":
        output += "\n**Errors:**\n" + errors
    msg = await edit_or_reply(nandeevent, output)
    await event.client.reload(msg)
...
async def _(event):  # sourcery no-metrics
    ...
    result = f"__Successfully disabled __ `{count}` __ for NandeUserbot sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
...
nandeub.loop.create_task(_init())
