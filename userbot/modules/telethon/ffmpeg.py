...
from ...helpers import _nandetools, media_type, progress, reply_id

plugin_category = "plugins"


FF_MPEG_DOWN_LOAD_MEDIA_PATH = os.path.join(
    Config.TMP_DOWNLOAD_DIRECTORY, "nandeuserbot.media.ffmpeg"
)
...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            nandeevent = await edit_or_reply(event, "`Saving the file...`")
            try:
                c_time = time.time()
                ...
                await event.client.fast_download_file(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, nandeevent, c_time, "trying to download")
                    ),
                )
                dl.close()
...
async def ff_mpeg_trim_cmd(event):
    ...
    nandeevent = await edit_or_reply(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await edit_delete(
                nandeevent, f"**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, nandeevent, c_time, "trying to upload")
                ),
            )
            os.remove(o)
        except Exception as e:
            return await edit_delete(nandeevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        # output should be image
        cmd, start_time = cmt
        o = await _nandetools.take_screen_shot(FF_MPEG_DOWN_LOAD_MEDIA_PATH, start_time)
        if o is None:
            return await edit_delete(
                nandeevent, f"**Error : **`Can't complete the process`"
            )
        ...
        await edit_delete(nandeevent, "RTFM")
        return
    end = datetime.now()
    ms = (end - start).seconds
