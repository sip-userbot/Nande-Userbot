...
from . import BOTLOG, BOTLOG_CHATID, edit_or_reply, logging, nandeub, spamwatch

LOGS = logging.getLogger(__name__)
plugin_category = "plugins"
if Config.ANTISPAMBOT_BAN:

    @nandeub.on(ChatAction())
    async def anti_spambot(event):  # sourcery no-metrics
        if not event.user_joined and not event.user_added:
            return
        user = await event.get_user()
        nandedmin = await is_admin(event.client, event.chat_id, event.client.uid)
        if not nandedmin:
            return
        nandebanned = None
        adder = None
        ...
        if is_gbanned(user.id):
            nandebanned = get_gbanuser(user.id)
            if nandebanned.reason:
                hmm = await event.reply(
                    f"[{user.first_name}](tg://user?id={user.id}) was gbanned by you for the reason `{nandegban.reason}`"
                )
            else:
                hmm = await event.reply(
            ...
            try:
                ...
                LOGS.info(e)
        if spamwatch and not nandebanned:
...
async def caschecker(event):
    ...
    try:
        ...
        await nandeevent.edit("`CAS check failed: Admin privileges are required`")
        return
    except BaseException:
        await nandeevent.edit("`CAS check failed`")
        return
...
async def caschecker(event):
    ...
    try:
        ...
        await nandeevent.edit("`spamwatch check failed: Admin privileges are required`")
        return
    except BaseException:
        await nandeevent.edit("`spamwatch check failed`")
        return
    await nandeevent.edit(text)
