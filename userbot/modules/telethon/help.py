...
async def grpinfo():
    outstr = "**Plugins in Nande-Userbot are:**\n\n"
    outstr += f"**👤 Owner : ** {mention}\n"
    outstr += f"**📜 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    category = ["modules", "plugins", "music"]
    for nande in category:
        plugins = GRP_INFO[nande]
        outstr += f"**{hemojis[nande]} {nande.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your nandeuserbot are :**\n\n"
    category = ["modules", "plugins", "music"]
    for nande in category:
        plugins = GRP_INFO[nande]
        outstr += f"**{hemojis[nande]} {nande.title()} ** - {len(plugins)}\n\n"
...
@nandeub.sip-userbot_cmd(
    ...
    info={
        "header": "To get guide for Nande-Userbot.",
...
async def _(event):
    ...
    if not input_str:
        ...
        outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help -c <command name>`"
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of Nandeuserbot are :"
    )
...
async def _(event):
    ...
    if found:
        ...
        out = f"I can't find any such command `{cmd}` in NandeUserbot"
