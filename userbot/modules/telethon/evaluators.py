...
@nandeub.sipuserbot_cmd(
    ...
    info={
        ...
        "examples": "{tr}exec nande stringsetup.py",
    },
)
async def _(event):
    ...
    nandeevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    nandeuser = await event.client.get_me()
    curruser = nandeuser.username or "nandeuserbot"
    uid = os.geteuid()
    if uid == 0:
...
@nandeub.sipuserbot_cmd(
    ...
    info={
        ...
        "examples": "{tr}eval print('nandeuserbot')",
    },
)
async def _(event):
    ...
    nandeevent = await edit_or_reply(event, "`Running ...`")
