...
from ...helpers.utils import _nandeutils
from . import BOTLOG, BOTLOG_CHATID

plugin_category = "plugins"


async def spam_function(event, sandy, nande, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(nande[0])
    if len(nande) == 2:
        spam_message = str(nande[1])
...
async def spammer(event):
    ...
    nande = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    counter = int(nande[0])
    if counter > 50:
        sleeptimet = 0.5
        sleeptimem = 1
...
@nandeub.sipuserbot_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using nandeuserbot",
    },
)
async def tmeme(event):
...
async def spammer(event):
    ...
    nande = input_str[1:]
    await event.delete()
    await spam_function(event, reply, nande, sleeptimem, sleeptimet, DelaySpam=True)
