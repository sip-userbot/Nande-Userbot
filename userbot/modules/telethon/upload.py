...
async def nandelst_of_files(path):
    files = []
...
def sortthings(contents, path):
    ...
    for file in contents:
        nandepath = os.path.join(path, file)
        if os.path.isfile(nandepath):
            nandesort.append(file)
    for file in contents:
        nandepath = os.path.join(path, file)
        if os.path.isdir(nandepath):
            nandesort.append(file)
    return nandesort
...
async def upload(path, event, udir_event, nandeflag=None):  # sourcery no-metrics
    nandeflag or False
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            nandepath = os.path.join(path, file)
            await upload(Path(nandepath), event, udir_event)
