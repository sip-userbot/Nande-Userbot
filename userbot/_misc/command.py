...
class userbot(object):
    ...
    def cute(
        **args,
        ...
        def decorator(func):
            if NandeBot:
            NandeBot.add_event_handler(func, events.NewMessage(**args))
            if NandeBot2:
                NandeBot2.add_event_handler(func, events.NewMessage(**args))
            if NandeBot3:
                NandeBot3.add_event_handler(func, events.NewMessage(**args))
            return func
        return decorator
    ...
    """Register a new event."""
        ...
        def decorator(func):
            async def wrapper(check):
                ...
                # with a dumb except.
                    ...
                    if not disable_errors:
                        ...
                        text = "**!!! Nande - Userbot ERROR REPOR !!!**\n\n"
            ...
            if NandeBot:
                if not disable_edited:
                    NandeBot.add_event_handler(wrapper, events.MessageEdited(**args))
                NandeBot.add_event_handler(wrapper, events.NewMessage(**args))
            if NandeBot2:
                if not disable_edited:
                    NandeBot2.add_event_handler(wrapper, events.MessageEdited(**args))
                NandeBot2.add_event_handler(wrapper, events.NewMessage(**args))
            if NandeBot3:
                if not disable_edited:
                    NandeBot3.add_event_handler(wrapper, events.MessageEdited(**args))
                NandeBot3.add_event_handler(wrapper, events.NewMessage(**args))
                return wrapper
