...
async def autogrup():
    from userbot import NandeBot as mansizbot
...
async def autobot():
    ...
    if who.username:
        ...
        username = "Nande-Userbot_" + (str(who.id))[5:] + "_bot"
    bf = "Botfather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "Nande-Userbot" + (str(who.id))[6:] + str(ran) + "_bot"
        await mansizbot.send_message(bf, username)
        await asyncio.sleep(1)
        ...
        if nowdone.startswith("Done!"):
            ...
            await mansizbot.send_message(bf, f"@{username}")
            await asyncio.sleep(1)
            await mansizbot.send_file(bf, "NandeVersion/Nande/Nandeasis.jpg")
            ...
            bf, f"🙋 Hello ✨ Saya Nande-Userbot Assistant"
            )
            await asyncio.sleep(2)
            ...
            await asyncio.sleep(1)
            await mansizbot.send_message(
            bf, f"Nande-Userbot Assistant\n\nBy ~ @Klyuserbot\nSupport ~ @Nandesupport ",
            )
            ...
            photo = await download_file(
                "https://telegra.ph/file/f64ad0d2f36ef2cfce84a.jpg", "channelphoto.jpg"
           )
            ll = await mansizbot.upload_file(photo)
            await mansizbot(EditPhotoRequest(chat_id, InputChatUploadedPhoto(ll)))
            os.remove(photo)
